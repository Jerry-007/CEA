...
const Post = ({ post }) => {
  ...
  const [likedPosts, setLikedPosts] = likedPostsState;
  const [dislikedPosts, setDislikedPosts] = dislikedPostsState;

  useEffect(() => {
    if (likedPosts.length > 0) {
      let found = false;
      for (let i = 0; i < likedPosts.length; i++) {
        if (likedPosts[i].id === post.id) {
          found = true;
          setLiked(found);
        }
      }
    ...
    if (dislikedPosts.length > 0) {
      ...
      for (let i = 0; i < dislikedPosts.length; i++) {
        if (dislikedPosts[i].id === post.id) {
          found = true;
          setDisliked(found);
        }
      }
    }
  }, [likedPosts, dislikedPosts, post.id]);
    ...
    <div className="card">
      <div className="card-body">
        ...
        <div className="d-flex justify-content-between">
          ...
          <div>
            <Link to={`/editpost/${post.id}`} className="btn c-btn me-3">
              Edit
            </Link>
            <button
              onClick={() => {
